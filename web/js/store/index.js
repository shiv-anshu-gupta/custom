...
import { STANDARDS, DEFAULT_EQUATIONS, BASE_CHANNELS, ADDITIONAL_CHANNELS } from '../../shared/standards.js';
 ...
 */
function buildInitialChannels(standardId = '9-2LE') {
    ...
    return channelOrder.map(id => {
        ...
        return {
            ...
            unit: base?.unit || '',
            equation: DEFAULT_EQUATIONS[id] || '0',
            isBase: true,
...
const initialData = {
    ...
    channels: buildInitialChannels('9-2LE'),
...
class Store {
    constructor() {
        ...
        this._data = {
            ...
            channels: buildInitialChannels(this._config.standard)
        };
     ...
     * @param {string} path - Dot-separated path like 'config.frequency' or 'data.stats.packetsSent'
     */
    get(path) {
        const parts = path.split('.');
        let value = parts[0] === 'config' ? this._config : this._data;
    ...
    _rebuildChannelsForStandard(standardId, standardConfig) {
        ...
        const newChannels = channelOrder.map(id => {
            ...
            return {
                ...
                phase: base?.phase || null,
                unit: base?.unit || '',
                equation: existing?.equation || DEFAULT_EQUATIONS[id] || '0',
                isBase: true,
                description: `Base ${base?.type || 'channel'} ${id}`
            };
        });
    ...
    _notifyChanges(changedPaths) {
        ...
        for (const path of changedPaths) {
            ...
            this._subscribers.get(`${root}.*`)?.forEach(cb => {
                if (!notified.has(cb)) {
                    cb(root === 'config' ? this._config : this._data, path);
                    notified.add(cb);
                }
            });
        }
    ...
    _generatePresetChannels(maxCount) {
        ...
        const testChannelPatterns = [
            ...
            { id: 'Va_h3', label: 'Vₐ_h3', equation: '32.5 * sin(6 * PI * 50 * t)', description: '3rd Harmonic Voltage A' },
            { id: 'Va_h5', label: 'Vₐ_h5', equation: '19.5 * sin(10 * PI * 50 * t)', description: '5th Harmonic Voltage A' },
            { id: 'Va_h7', label: 'Vₐ_h7', equation: '13 * sin(14 * PI * 50 * t)', description: '7th Harmonic Voltage A' },
            
            // Current harmonics
            { id: 'Ia_h3', label: 'Iₐ_h3', equation: '10 * sin(6 * PI * 50 * t)', description: '3rd Harmonic Current A' },
            { id: 'Ia_h5', label: 'Iₐ_h5', equation: '6 * sin(10 * PI * 50 * t)', description: '5th Harmonic Current A' },
...
export { store, initialConfig, initialData, buildInitialChannels, STANDARDS, BASE_CHANNELS, ADDITIONAL_CHANNELS };
